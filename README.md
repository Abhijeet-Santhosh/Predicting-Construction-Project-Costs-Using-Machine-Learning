...
All code, dataset, and outputs can be accessed via the following links:  
...
India’s construction and real estate sector is one of the country’s largest economic contributors, yet it faces persistent cost overruns due to inaccurate early-stage estimates. Traditional regression techniques often fail to capture the nonlinear cost interactions that influence real-world project variability.

This project develops a Random Forest–based machine learning model that predicts construction costs for RERA-registered projects in Gujarat with high reliability. The model is able to explain **95%** of the variance in project costs, achieving strong RMSE and MAE scores, which indicate excellent predictive accuracy. **Trained on 14,507 projects**, it outperforms traditional regression methods and identifies key cost drivers such as promoter profile, carpet area, and land cost, providing a robust and scalable framework for improving early-stage budgeting and cost transparency across India’s real estate sector.
